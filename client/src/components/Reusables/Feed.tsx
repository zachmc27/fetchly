...
export default function Feed({
  ...
  useEffect(() => {
    ...
    if (storedConversationId && location.pathname === "/inbox") {
      ...
      const storedConversation = mockConversations.find(
...
const { data, loading, error } = useQuery(GET_CONVERSATION, {
  ...
  function handleMessagePageRender(messageId: number) {
    const conversationToOpen = mockConversations.find(
      (convo) => convo.id === messageId
    );

    if (conversationToOpen) {
      console.log(
        "Opening chat for conversation:",
        conversationToOpen.conversationName
      );
      setActiveConversation(conversationToOpen);
      setIsChatOpen(true);
      localStorage.setItem(
        "activeConversationId",
        conversationToOpen.id.toString()
      );
    } else {
  ...
  // ----------------------------------------------------------------
  const [activeMeetupPost, setActiveMeetupPost] =
    useState<MockMeetupItem | null>(null);
  ...
  useEffect(() => {
    ...
    if (storedMeetupId && location.pathname === "/meetup") {
      ...
      const storedMeetup = mockMeetupPosts.find(
  ...
  function handleMeetupViewRender(meetupId: string) {
    const meetupToOpen = mockMeetupPosts.find(
      (post) => post.id.toString() === meetupId
    );
  ...
  useEffect(() => {
    ...
    if (storedPostId && location.pathname === "/") {
      const storedPost = initialFeedArray.find(
        (post) =>
          post.itemType === "post" && (post as PostCard)._id === storedPostId
  ...
  // ----------------------------------------------------------------
  const [activeAdoptionPost, setActiveAdoptionPost] =
    useState<MockAdoptionItem | null>(null);
  const [isAdoptionPostOpen, setIsAdoptionPostOpen] = useState(false);

  function handleAdoptionPostViewRender(adoptionId: string) {
    const adoptionToOpen = mockAdoptionPosts.find(
      (adoption) => adoption._id === adoptionId
    );
          ...
          >
            ...
            {messageItem.coverImage ? (
              ...
              <div className="message-icon">
                <img src={group} alt="Chat icon" className="default-icon" />
              </div>
            )}
      ...
      case "adoption": {
        ...
        return (
          <div
            ...
            className={itemStyle}
            onClick={() => handleAdoptionPostViewRender(adoptionItem._id)}
          >
            <div className="adoption-image-container">
              <img
                src={
                  adoptionItem.pet.profilePhoto?.url ||
      ...
      case "meetup": {
        ...
        return (
          <div
            key={index}
            className={itemStyle}
            onClick={() => handleMeetupViewRender(meetupItem._id)}
          >
    ...
    return (
      <>
        ...
        <div className="tab-switcher">
          <button
            onClick={handleGoingListRender}
            className={activeTab === "going" ? "active" : ""}
          >
            Going
          </button>
          <button
            onClick={handleCommentsRender}
  ...
  return (
    <>
      ...
      <div className={containerStyle}>{feedArray.map(renderFeedItem)}</div>
      <PostButton />
    </>
  );
}
